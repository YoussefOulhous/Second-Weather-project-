...
function Main({darkMode,homeRef,scrollToSection,cartRef}) {
  return (
    <div className={`${darkMode && "dark"} bg-white dark:bg-neutral-900`}>
      ...
      wrapper="span"
      speed={50}
      className="text-7xl font-poppins w-[400px] font-semibold tracking-wide sm:text-5xl md:text-6xl dark:text-yellow-400 dark:shadow-xl"
      repeat={Infinity}
    />
          ...
          <button className="w-40 py-4 mt-10 border-2 bg-neutral-800 rounded-xl text-white hover:bg-slate-950 dark:hover:bg-neutral-900 dark:hover:text-white dark:hover:border-white sm:hover:bg-slate-950  md:hover:bg-slate-950 dark:bg-white dark:text-neutral-900"  onClick={()=>{scrollToSection(cartRef)}}>
            Weather Cart
          </button>
